...
import { DashboardService, ChartConfiguration, DashboardError } from '../lib/dashboardService';
...
const ChatMode = ({ 
  ...
  const [selectedChartType, setSelectedChartType] = React.useState<'line' | 'bar' | 'pie'>('line');
  ...
  React.useEffect(() => {
    if (profile && accessLevels.length > 0) {
      const userAccessLevel = accessLevels.find(level => level.id === profile.access_level_id);
      if (userAccessLevel) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (message.trim()) {
      ...
      try {
        ...
        if (!session) {
          session = await ChatQueryService.createSession(`Chat ${new Date().toLocaleString()}`);
          setCurrentSession(session);
        }
        
        // If the message asks about tables, show the data table
        if (message.toLowerCase().includes('table') || message.toLowerCase().includes('show me')) {
          const { data: tableData, error: tableError } = await supabase
            .from(message.toLowerCase().includes('orders') ? 'orders' : 'customers')
            .select('*')
            .limit(100);
          ...
          if (!tableError && tableData) {
            ...
            setChatHistory(prev => [...prev, { 
              role: 'assistant', 
              content: `Here's the data from the ${message.toLowerCase().includes('orders') ? 'orders' : 'customers'} table:`,
              table: {
                columns,
                data: tableData
              }
...
But there was an error: ${queryError instanceof Error ? queryError.message : 'Unknown error'}
            `.trim();
    ...
    try {
      ...
      const configuration: ChartConfiguration = {
        ...
        datasets: [{
          ...
          backgroundColor: ['#167147', '#4E7BE9', '#4EE997', '#E94E7B', '#7BE94E', '#E9974E'],
          borderColor: '#167147'
        }]
      };
      ...
      setChatHistory(prev => [...prev, {
        role: 'assistant',
        content: `Chart has been successfully saved to ${view === 'dashboard' ? 'the' : 'a new'} dashboard! ${
      ...
      setSaveError(error instanceof DashboardError ? error.message : 'Failed to save chart to dashboard');
    } finally {
  ...
  const handleMentionSelect = (column: { table: string; name: string }) => {
    ...
    if (lastAtIndex !== -1) {
      const newMessage = message.slice(0, lastAtIndex) + `@${column.table}.${column.name} `;
  ...
  return (
    <div className="chat-container">
          ...
          <div className="chat-content">
            <div className="messages">
              {chatHistory.map((msg, index) => (
                <div key={`${msg.role}-${index}`} className={`message ${msg.role === 'user' ? 'user' : 'ai'}`}>
                        ...
                        {msg.visualization.type === 'metric' ? (
                          ...
                          <div className="chart-container">
                            {msg.visualization.type === 'line' && (
                              <Line data={msg.visualization.data} options={chartOptions} />
                            )}
                            {msg.visualization.type === 'bar' && (
                              <Bar data={msg.visualization.data} options={chartOptions} />
                            )}
                            {msg.visualization.type === 'pie' && (
                              <Pie data={msg.visualization.data} options={chartOptions} />
                            )}
                          </div>
                        )}
                      </div>
                    )}
                    {msg.role === 'assistant' && executionTime && index === chatHistory.length - 1 && (
                      <div className="execution-time">
                        Executed in {(executionTime / 1000).toFixed(2)}s
                      </div>
                    )}
                    {msg.role === 'assistant' && lastQueryResult && index === chatHistory.length - 1 && (
                      <div className="results-table">
                        <table>
                          <thead>
                            <tr>
                              {Object.keys(lastQueryResult.data[0] || {}).map((column, index) => (
                          ...
                          <tbody>
                            {lastQueryResult.data.map((row, rowIndex) => (
                              <tr key={rowIndex}>
                                {Object.keys(lastQueryResult.data[0] || {}).map((column, colIndex) => (
      ...
      {view !== 'chat' && (
        view === 'metrics' ? (
          <MetricsView profile={profile} title={metricsTitle} isNew={isNewMetric} />
        ) : (
          <DashboardView profile={profile} title={dashboardTitle} isNew={isNewDashboard} />
        )
      )}
