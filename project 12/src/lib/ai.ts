...
const OPENROUTER_KEY = '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';
...
export function parseAIResponse(response: string): ParsedAIResponse {
  const explanationMatch = response.match(/---EXPLANATION---([\s\S]*?)(?:---SQL---|$)/);
  const sqlMatch = response.match(/---SQL---([\s\S]*?)---END SQL---/);
  const chartMatch = response.match(/---CHART---([\s\S]*?)---END CHART---/);
  
  return {
    explanation: explanationMatch ? explanationMatch[1].trim() : response.trim(),
